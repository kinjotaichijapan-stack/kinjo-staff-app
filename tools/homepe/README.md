...
| 画面 | URL | 取得する値 |
| --- | --- | --- |
| 月次在庫同期カレンダー | `ydf_zaikocalendar.html` | 部屋タイプ別・日別の**在庫残数（共通在庫）**・最大部屋提供数 |
| 統計・分析(期間) | `ydf_rsvgraph.html` | 申込経路別の売上・客単価 |
| 予約・残室状況 | `ydf_reserveweek.html` | （参考）物理残室。`--source reserveweek` 指定時のみ |

> **在庫について**: 残室（在庫残）は在庫カレンダーの「残数」から取得する。残数は共通在庫のため
> どのサイト（宿帳くん／楽天／Agoda…）でも同値で、部屋タイプ合計は予約・残室状況の残室と一致する
> （通常日・満室日・ブロック日で一致を確認済み）。既定の取得元は在庫カレンダー。
